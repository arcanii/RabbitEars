...
It lives in its own file so it can be pasted without hunting for it in here:
**[`mac/SEED_PROMPT.md`](SEED_PROMPT.md)** — the whole file IS the prompt, so
`cat mac/SEED_PROMPT.md` gives exactly what to paste, with nothing to trim.

**Keep it current.** It is the only thing a fresh session reads before this document, so when the
state changes (a release published, a branch merged, a new trap learned) update it in the same
commit — a stale seed prompt is worse than none, because it is trusted.
