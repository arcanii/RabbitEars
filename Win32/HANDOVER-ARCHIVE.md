...
# RabbitEars — Handover ARCHIVE (release history)

The **per-release detail** split out of [`HANDOVER.md`](HANDOVER.md) on 2026-07-26, when that file
reached 1,503 lines and the current state had become hard to find inside its own history.

**`HANDOVER.md` remains the single entry point** — it carries the current state, the immediate next
steps, and every durable rule (architecture, toolchain, release procedure, gotchas). Nothing here is
needed to work on the app today. This file exists so that "why was it done that way?" is still
answerable: each block below is the contemporaneous account of a release, kept verbatim rather than
summarised, because the reasoning is the part that turned out to be worth keeping.

Read it when you are about to re-litigate an old decision, and to check whether an idea has already
been tried and reverted.

> ⚠️ **Historical. Not maintained.** Line numbers, file paths, and "next steps" in these blocks were
> true when written and many are now wrong — `common/` was restructured, the theme engine shipped,
> and files have moved. Verify against the working tree before acting on anything here.

---

## Release detail — v0.2.13 and older

*(Split from `HANDOVER.md`'s "Current state" section. Newest first; runs back to v0.2.4 and then into
the 0.2.1 multi-view / EPG / theme-engine epics and the whole 0.1.x line.)*
...
## "Immediate next steps" — the shipped blocks, v0.2.13 and older

*(Split from `HANDOVER.md`'s "Immediate next steps" section: the per-release ship notes plus the long
feature write-ups that accumulated under them — localization, wake-to-record, the recording
scheduler, saved layouts, favourites I/O, the ARM64 port.)*
